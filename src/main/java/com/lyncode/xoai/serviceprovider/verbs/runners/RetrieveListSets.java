...
public class RetrieveListSets implements Runnable {
    ...
    public RetrieveListSets(ProcessingQueue<SetType> list, OAIServiceConfiguration<MetadataParser, AboutItemParser, DescriptionParser, AboutSetParser> config, Parameters params) {
        queue = list;
        this.config = config;
    ...
    public void run() {
        try {
            ...
            if (timeAfter - timeBefore < config.getIntervalBetweenRequests())
                Thread.sleep(config.getIntervalBetweenRequests() - (timeAfter - timeBefore));
