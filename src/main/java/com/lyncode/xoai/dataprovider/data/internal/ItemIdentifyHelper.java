    ...
    public List<ReferenceSet> getSets(XOAIContext context) {
        ...
        for (StaticSet staticSet : context.getStaticSets()) {
            if (staticSet.hasCondition() && staticSet.getCondition().getFilter().isItemShown(item))            {
                // this set has a condition and the current item fulfills it so add the set to the item
                list.add(staticSet);
            }
            ...
            // otherwise: the set has a condition which is not fulfilled by the current item so don't add.
        }
        return list;
    }
